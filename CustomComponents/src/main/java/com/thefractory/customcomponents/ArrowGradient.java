...
public class ArrowGradient extends CustomGradient {
	...
	= FXCollections.observableArrayList(new ArrayList<GradientMakerInterface>());
	...
	public ArrowGradient(ArrayList<ColorArrow> colorArrows) {
	...
	private void calculateGradientMakers() {
		...
		gradientMakerList.add(new GradientMakerInterface((Color) arrows.get(0).color.getFill(), 
				(Color) arrows.get(0).color.getFill(), 
				(int) Math.round(totalDepth* (arrows.get(0).position.doubleValue() 
						- 0)), "Linear"));
		
		for(int i = 0; i < arrows.size() - 1; i++) {
			gradientMakerList.add(new GradientMakerInterface((Color) arrows.get(i).color.getFill(), 
		...
		gradientMakerList.add(new GradientMakerInterface((Color) arrows.get(arrows.size() - 1).color.getFill(), 
	...
	public void updatePalette(){
		...
		for(GradientMakerInterface gradientMaker : this.gradientMakerList) {
	...
	public void newColorArrow(ColorArrow arrow) {
    	...
    	if(colorArrowList.size() > 1) {
    		removeSelectedArrow.setDisable(false);
    	}
		select(arrow);
	}
	...
	@FXML public void reverse() {
		for(ColorArrow arrow : colorArrowList) {
			arrow.position.set(1 - arrow.position.get());
		}
	}
	
	@FXML public void removeSelectedArrow() {
		if(colorArrowList.size() > 1) {
			ColorArrow selectedArrow = null;
			for(ColorArrow arrow : colorArrowList) {
				if(arrow.selected) {
					selectedArrow = arrow;
				}
			...
			colorArrowList.remove(selectedArrow);
			colorArrowContainer.getChildren().remove(selectedArrow);
			if(colorArrowList.size() == 1) {
				removeSelectedArrow.setDisable(true);
			}
		}
	}
